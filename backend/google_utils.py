...
def check_granted_scopes(user, db: Session):
    ...
    has_gmail = (
        'https://www.googleapis.com/auth/gmail.send' in granted_scopes or
        'https://www.googleapis.com/auth/gmail.modify' in granted_scopes
    )
...
def upload_template_attachments_to_drive(drive_service, attachments: list[dict], app_folder_id: str):
    """
...
def download_template_attachments_from_drive(drive_service, attachment_refs: list[dict]):
    """
...
def get_template_attachments(user, db: Session, attachment_refs: list[dict]):
    ...
    if attachment_refs and isinstance(attachment_refs[0], dict):
        if 'drive_file_id' in attachment_refs[0]:
            ...
            creds = ensure_creds(user, db)
            drive_service = build('drive', 'v3', credentials=creds)
            return download_template_attachments_from_drive(drive_service, attachment_refs)
...
def stage_bulk_image_sub_account(admin: EnterpriseAdmin, sub_account: SubAccount, db: Session, file_obj: bytes, filename: str):
    """
    ...
    try:
        ...
        ensure_sub_account_bulk_sheets(sheets_service, admin.google_spreadsheet_id, staging_sheet, queue_sheet)
...
def ensure_sub_account_bulk_sheets(service, spreadsheet_id: str, staging_sheet: str, queue_sheet: str):
...
def check_staging_count_for_user(admin, user, user_type: str, db: Session):
    ...
    try:
        ...
        if user_type == "sub_account":
            ...
            ensure_sub_account_bulk_sheets(service, admin.google_spreadsheet_id, staging_sheet, _)
        ...
        res = service.spreadsheets().values().get(
            spreadsheetId=admin.google_spreadsheet_id, range=sheet_range
        ).execute()
...
def submit_bulk_session_sub_account(admin: EnterpriseAdmin, sub_account: SubAccount, db: Session):
...
def clear_staging_data_sub_account(admin: EnterpriseAdmin, sub_account: SubAccount, db: Session):
...
def process_bulk_queue_sync_sub_account(
    ...
    while True:
        try:
            res = sheets_service.spreadsheets().values().get(
                spreadsheetId=admin.google_spreadsheet_id, range=f"{queue_sheet}!A1:B1"
            ).execute()
            rows = res.get('values', [])
            ...
            try:
                ...
                print(f"Failed to download {file_id}: {e}")
                delete_top_row_from_sheet(sheets_service, admin.google_spreadsheet_id, queue_sheet)
                continue
            ...
            if template and contact_data.get('email'):
                try:
                    ...
                    from backend.main import normalize_emails, generate_email_prompt, send_gmail
                    ...
                    if emails:
                        ...
                        for email_addr in emails:
                            try:
                                send_gmail(admin, db, email_addr, content['subject'], content['body'])
                            except: pass
                except: pass
...
def fetch_templates(user: User, db: Session):
    ...
    try:
        ...
        result = service.spreadsheets().values().get(
            spreadsheetId=user.google_spreadsheet_id, range=f"{TEMPLATE_SHEET_NAME}!A2:E"
        ).execute()
...
def add_template(user, db: Session, subject: str, body: str, attachments: list[dict] = None):
    """
    ...
    try:
        ...
        if attachments:
            ...
            attachment_refs = upload_template_attachments_to_drive(drive_service, attachments, app_folder_id)
...
def update_template_content(user: User, db: Session, row_id: int, subject: str, body: str, attachments: list[dict] = None):
    """
    ...
    try:
        ...
        sheets_service.spreadsheets().values().update(
            spreadsheetId=user.google_spreadsheet_id, range=f"{TEMPLATE_SHEET_NAME}!B{row_id}:E{row_id}",
            valueInputOption="USER_ENTERED", body={'values': [[subject, body, "", attachments_json]]}
        ).execute()
...
def send_gmail(user: User, db: Session, to_email: str, subject: str, body_html: str, attachments: list[dict] = None):
    """
...
def create_sub_account_sheet(admin: EnterpriseAdmin, sub_account: SubAccount, db: Session):
    """
    ...
    try:
        ...
        meta = service.spreadsheets().get(spreadsheetId=admin.google_spreadsheet_id).execute()
        existing_titles = [s['properties']['title'] for s in meta.get('sheets', [])]
...
def append_to_sub_account_sheet(admin: EnterpriseAdmin, sub_account: SubAccount, db: Session, row_data: list):
    """
...
def export_sheet_as_excel(admin: EnterpriseAdmin, db: Session, sheet_name: Optional[str] = None):
    """
    ...
    try:
        ...
        if sheet_name:
            ...
            meta = sheets_service.spreadsheets().get(spreadsheetId=admin.google_spreadsheet_id).execute()
            ...
            temp_ss = sheets_service.spreadsheets().create(body={'properties': {'title': f'Temp_{sheet_name}'}}).execute()
            temp_ss_id = temp_ss['spreadsheetId']

            # Copy sheet to temp spreadsheet
            copy_req = {
                'destinationSpreadsheetId': temp_ss_id
            }
            ...
            temp_meta = sheets_service.spreadsheets().get(spreadsheetId=temp_ss_id).execute()
            for s in temp_meta.get('sheets', []):
                if s['properties']['title'] == 'Sheet1':
                    delete_req = {'deleteSheet': {'sheetId': s['properties']['sheetId']}}
                    sheets_service.spreadsheets().batchUpdate(
                        spreadsheetId=temp_ss_id,
...
def export_combined_contacts(admin: EnterpriseAdmin, sub_accounts: List[SubAccount], db: Session):
    """
    ...
    try:
        ...
        temp_ss = sheets_service.spreadsheets().create(
            body={'properties': {'title': 'Temp_Combined_Contacts'}}
        ).execute()
        temp_ss_id = temp_ss['spreadsheetId']
        ...
        temp_meta = sheets_service.spreadsheets().get(spreadsheetId=temp_ss_id).execute()
        for s in temp_meta.get('sheets', []):
            if s['properties']['title'] == 'Sheet1' and len(temp_meta.get('sheets', [])) > 1:
